...
export const CONFIG = {
    ...
    // ⚠️ 2026-09-15 现网实测修正：原值 3000ms 是**必然失败**的。
    // 目标房间后台页必须存活到「用户级活动上下文」建立完成，关闭后才能由控制页 snatch 成功；
    // 否则 snatch 恒返回 12006「稍等会儿才能抢哟」。
    // 实测（同一账号 / 同一控制页 / 同样 waitSec=90，唯一变量是关页时机）：
    //   3s（原默认）→ 12006；8s → 12006；12s → error=0 成功；保持打开 → error=0 成功。
    // 与上游研究记录吻合：红包组件约需 5.3~8.1 秒才渲染出来。
    ROOM_PREWARM_DURATION: 12000,
